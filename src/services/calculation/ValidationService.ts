...
export class ValidationService implements IValidationService {
	...
	validateInput(
		...
		for (const {component, minPct} of normalizedComponents) {
			...
			const available = this.totalAvailableForComponent(
				component,
				normalizedInv
			);
			...
			if (available < minMb) {
				return {
					...
					error: {
						...
						statusContext:
							`Not enough ${component} for minimum requirement (` +
							`${minMb - available}` +
							'mB or ' +
							`${(
								minMb -
								available -
								Math.floor(minMb - available) <=
								1.0
							) ?
								(
									(minMb - available) /
		...
		if (totalAvailableFromRecipe < targetMb) {
			return {
				...
				error: {
					...
					statusContext:
						'Not enough total material available (' +
						+`${targetMb - totalAvailableFromRecipe}` +
						'mB or ' +
						`${(
							targetMb -
							totalAvailableFromRecipe -
							Math.floor(
								targetMb - totalAvailableFromRecipe
							) <=
							1.0
