...
describe('ValidationService', () => {
	...
	describe('validateInput', () => {
		it('should return valid for correct input', () => {
			...
			const result = sut.validateInput(
				1000,
		...
		it('should reject non-integer targetMb', () => {
			...
			testCases.forEach(({target}) => {
				...
				expect(result.error?.statusContext).toContain(
					'positive integer'
				);
			});
		});
		...
		it('should reject insufficient specific mineral', () => {
			...
			const normalizedInv = new Map([
				...
				[
					'copper',
					[createQuantifiedMineral('Copper Ore', 'copper', 100, 1)]
				] // 100mB total
			]);
			...
			expect(result.error?.statusContext).toContain(
				'Not enough copper for minimum requirement'
			);
		});
		...
		it('should handle multiple components validation', () => {
			...
			const normalizedInv = new Map([
				...
				[
					'copper',
					[createQuantifiedMineral('Copper Ore', 'copper', 100, 6)]
				] // 600mB
			]);
