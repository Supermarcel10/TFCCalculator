...
import {
	createComponent,
	createQuantifiedMineral,
	expectUsedToNotExceedAvailable
} from '@test/helpers';
import {
	Flags,
	OutputCode
...
describe('OutputCalculator', () => {
	...
	describe('calculateSmeltingOutput', () => {
		...
		it('should return unfeasible when no combination exists', () => {
			...
			const availableMinerals = new Map([
				...
				[
					'copper',
					[createQuantifiedMineral('Copper Ore', 'copper', 100, 5)]
				] // 500 mB
			]);
			...
			expect(result.status).toBe(
				OutputCode.INSUFFICIENT_SPECIFIC_MINERAL_MB
			);
			expect(result.statusContext).toContain(
				'Not enough iron for minimum requirement (300mB or 2 ingot(s) short)'
			);
		});
		...
		it('should handle complex multi-component scenario', () => {
			...
			const availableMinerals = new Map([
				...
				[
					'copper',
					[createQuantifiedMineral('Copper Ore', 'copper', 100, 10)]
				], // 1000 mB
	...
	describe('edge cases', () => {
		it('should handle zero-yield minerals', () => {
			...
			expect(result.statusContext).toBe(
				'Not enough total material available (100mB or 1 ingots short)'
			);
		});
		...
		it('should handle very large quantities efficiently', () => {
			...
			const availableMinerals = new Map([
				[
					'iron',
					[createQuantifiedMineral('Iron Ore', 'iron', 100, 1000)]
				] // 100_000 mB
			]);
	...
	describe('CLOSEST_ALTERNATIVE flag', () => {
		...
		it('should find next higher interval when exact match fails', () => {
			...
			const availableMinerals = new Map([
				...
				[
					'zinc',
					[createQuantifiedMineral('Rich Sphalerite', 'zinc', 42, 6)]
				] // 252 mB
			]);
